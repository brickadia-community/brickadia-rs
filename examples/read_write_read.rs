...
use brickadia::{read::SaveReader, write::SaveWriter};
...
fn main() {
    ...
    SaveWriter::new(File::create("examples/read.out.brs").unwrap(), save)
        .write()
        .unwrap();
    let mut reader = SaveReader::new(File::open("examples/read.out.brs").unwrap()).unwrap();
